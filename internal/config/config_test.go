...
// Phase 16: Custom Fields Configuration Tests

func TestConfig_CustomFieldsFromFile(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, ".jira-sync.yaml")
	configContent := `
jira:
...
`
	...
	require.NoError(t, err)
	assert.Equal(t, "customfield_10015", cfg.CustomFields.StartDate)
	assert.Equal(t, "customfield_10016", cfg.CustomFields.EndDate)
}

func TestConfig_CustomFieldsFromEnv(t *testing.T) {
	t.Setenv("JIRA_URL", "https://test.atlassian.net")
	t.Setenv("JIRA_USER", "test@test.com")
	t.Setenv("JIRA_TOKEN", "test-token")
	t.Setenv("JIRA_CUSTOM_FIELDS_START_DATE", "customfield_10020")
	t.Setenv("JIRA_CUSTOM_FIELDS_END_DATE", "customfield_10021")

	cfg, err := LoadFromEnv()

	require.NoError(t, err)
	assert.Equal(t, "customfield_10020", cfg.CustomFields.StartDate)
	assert.Equal(t, "customfield_10021", cfg.CustomFields.EndDate)
}

func TestConfig_CustomFieldsDefaults(t *testing.T) {
	t.Setenv("JIRA_URL", "https://test.atlassian.net")
	t.Setenv("JIRA_USER", "test@test.com")
	t.Setenv("JIRA_TOKEN", "test-token")
	...
	require.NoError(t, err)
	// Custom fields should be empty when not configured (no defaults)
	assert.Empty(t, cfg.CustomFields.StartDate)
	assert.Empty(t, cfg.CustomFields.EndDate)
}
